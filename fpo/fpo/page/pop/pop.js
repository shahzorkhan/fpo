...
frappe.pages['pop'].refresh = function (wrapper) {
	window.onbeforeunload = function () {
		return wrapper.pop.beforeunload()
	}
}

fpo.pop.PointOfPurchase = erpnext.taxes_and_totals.extend({
	...
	beforeunload: function (e) {
		if (this.connection_status == false && frappe.get_route()[0] == "pop") {
			e = e || window.event;
	...
	make_menu_list: function () {
		...
		this.page.add_menu_item(__("New Farmer Quotation"), function () {
		...
		this.page.add_menu_item(__("Sync Offline Invoices"), function () {
			me.sync_farmer_quotation()
		});

		// this.page.add_menu_item(__("pop Profile"), function () {
			// frappe.set_route('List', 'pop Profile');
		// });
	},
	...
	render_offline_data: function() {
		...
		$(this.list_body).append('<div class="row list-row list-row-head pop-invoice-list">\
				<div class="col-xs-1"><input class="list-select-all" type="checkbox"></div>\
				<div class="col-xs-3">Farmer</div>\
		...
		$.each(this.fq_docs, function(index, data){
			for(key in data) {
				$(frappe.render_template("pop_invoice_list", {
					sr: index + 1,
					name: key,
					customer: data[key].farmer,
	...
	dialog_actions: function () {
		...
		$(this.list_body).find('.list-select-all').click(function () {
			...
			if ($(this).is(":checked")) {
				$.each(me.fq_docs, function (index, data) {
					for (key in data) {
	...
	edit_record: function () {
		...
		doc_data = this.get_invoice_doc(this.fq_docs);
		if (doc_data) {
	...
	get_invoice_doc: function (fq_docs) {
		var me = this;
		this.fq_docs = this.get_doc_from_localstorage();

		return $.grep(this.fq_docs, function (data) {
			for (key in data) {
				return key == me.name
			}
		})
	...
	get_data_from_server: function (callback) {
		var me = this;
		frappe.call({
			method: "fpo.fpo.doctype.farmer_quotation.pop.get_pop_data",
			freeze: true,
...
//		this.letter_head = (this.pos_profile_data.length > 0) ? frappe.boot.letter_heads[this.pos_profile_data[letter_head]] : {};
	...
	load_data: function (load_doc) {
		...
		this.print_template_data = frappe.render_template("print_template", {
			...
			title: "pop", base_url: frappe.urllib.get_base_url(), print_css: frappe.boot.print_css,
			print_settings: this.print_settings,// header: this.letter_head.header, footer: this.letter_head.footer
		})
	},

	setup: function () {
		this.frm.doc.allow_delete = true;//this.pos_profile_data["allow_delete"];
		// TODO: Temporary Hack
		this.frm.doc.apply_discount_on = false;
		this.wrapper.html(frappe.render_template("pop", this.frm.doc));
		// End Hack
		this.set_transaction_defaults("Farmer");
		this.make();
	...
	make_search: function () {
		...
		this.search_item.$input.on("keyup", function () {
			setTimeout(function () {
		...
		this.quotation_type_field = frappe.ui.form.make_control({
			df: {
				"fieldtype": "Select",
				"options": this.meta.quotation.fields.find(
				    function(field){
				        return field.fieldname == 'quotation_type';
				        }
                    ).options,
				"label": __("Quotation Type"),
				"fieldname": "quotation_type",
				"placeholder": __("Quotation Type")
			},
			parent: this.wrapper.find(".quotation_type"),
			only_input: true,
		});

		this.quotation_type_field.make_input();
		this.quotation_type_field.$input.on("change", function () {
	...
	make_list_farmers: function () {
		var me = this;
		this.list_farmers_btn = this.wrapper.find('.list-customers-btn');
		this.add_farmer_btn = this.wrapper.find('.add-customer-btn');
		this.pop_bill = this.wrapper.find('.pop-bill').hide();
		this.list_farmers = this.wrapper.find('.list-customers');

		this.list_farmers_btn.on('click', function () {
			$(this).toggleClass("view_customer");
			if($(this).hasClass("view_customer")) {
				me.render_list_farmers();
				me.bind_delete_event()
				me.party_field.$input.attr('disabled', true);
				me.list_farmers.show();
				me.pop_bill.hide();
			} else {
				...
				me.pop_bill.show();
				me.list_farmers.hide()
			}
		});
		this.add_farmer_btn.on('click', function() {
			me.save_previous_entry();
			me.create_new();
			me.refresh();
			me.set_focus();
	...
	render_list_farmers: function () {
		var me = this;

		this.removed_items = [];
		this.list_farmers.empty();
		this.fq_docs = this.get_doc_from_localstorage();

		if (!this.fq_docs.length) {
			this.list_farmers.append(
				'<div style="padding: 12px; margin-left:-12px;">' + __("No offline records.") + '</div>'
			)
			return;
		}
		var html = '<div class="row list-row list-row-head pop-invoice-list">\
			<div class="col-xs-1"><input class="list-select-all" type="checkbox"></div>\
			<div class="col-xs-3">Farmer</div>\
			<div class="col-xs-2 text-left">Status</div>\
			<div class="col-xs-3 text-right">Paid Amount</div>\
		...
		this.fq_docs.forEach(function (data, i) {
			for (key in data) {
				html += frappe.render_template("pop_invoice_list", {
					sr: i + 1,
					name: key,
					farmer: data[key].farmer,
					paid_amount: format_currency(data[key].paid_amount, me.frm.doc.currency),
					grand_total: format_currency(data[key].grand_total, me.frm.doc.currency),
					data: me.get_doctype_status(data[key])
				});
		...
		this.list_farmers.append(html);

		this.list_farmers.find('.list-column').click(function () {
			me.list_farmers.hide();
			me.list_farmers_btn.toggleClass("view_customer");
			me.pop_bill.show();
			me.list_farmers_btn.show();
			me.name = $(this).parents().attr('invoice-name')
			me.edit_record();
		})
		...
		$(this.wrapper).find('.list-select-all').click(function () {
			me.list_farmers.find('.list-delete').prop("checked", $(this).is(":checked"))
			me.removed_items = [];
			if ($(this).is(":checked")) {
				$.each(me.fq_docs, function (index, data) {
					for (key in data) {
	...
	make_farmer: function () {
		...
		var farmers = this.farmers.map(function (c) {
			return {
				label: c.title,
				value: c.name,
		...
		this.party_field.$input
			.on('input', function (e) {
				me.party_field.awesomeplete.list = farmers;
			})
			.on('awesomplete-select', function (e) {
				var farmer = me.party_field.awesomeplete
					.get_item(e.originalEvent.text.value);
				if (!farmer) return;
				// create farmer link
				if (farmer.action) {
					farmer.action.apply(me);
					return;
				}
				me.update_farmer_data(farmer);
				me.refresh();
				me.set_focus();
			})
			.on('change', function (e) {
				if (!e.originalEvent.text) {
					me.frm.doc.farmer = $(this).val();
				}
			})
			.on('focus', function (e) {
	...
	new_farmer: function () {
		...
		this.farmer_doc = new frappe.ui.Dialog({
			'title': 'Farmer',
			fields: [
				{
					"label": __("Name"),
		...
		this.farmer_doc.show()

		this.farmer_doc.set_primary_action(__("Save"), function () {
			me.make_offline_farmer();
			me.pop_bill.show();
		});
	},
	...
	make_offline_farmer: function() {
		...
		this.party_field.$input.val(this.frm.doc.farmer);
		this.farmers.push({
			name: this.frm.doc.farmer,
			farmer_name: this.frm.doc.farmer
		});

		this.farmer_doc.hide()
	},

	update_farmer_data: function (doc) {
		var me = this;
	...
	get_farmers: function (key) {
		...
		if (key) {
			return $.grep(this.farmers, function (data) {
				if (reg.test(data.name.toLowerCase())
					|| reg.test(data.farmer_name.toLowerCase())
			...
			farmers = this.farmers.sort(function (a, b) { return a.idx < b.idx })
	...
	make_item_list: function () {
		...
		if (this.items.length > 0) {
			$.each(this.items, function(index, obj) {
				if(index < me.page_len){
					$(frappe.render_template("pop_item", {
						item_code: obj.name,
						item_price: format_currency(0 /*me.price_list_data[obj.name]*/, me.frm.doc.currency),
		...
		$(me.wrapper).find("div.pop-item").on("click", function () {
			if(me.list_farmers_btn.hasClass("view_customer")) return;
	...
	get_items: function (item_code)
	{
		...
		this.items_list = this.item_data;//this.apply_category();
			...
			return $.grep(this.items_list, function (item) {
				if (search_status) {
					/*if (in_list(me.batch_no_data[item.item_code], me.search_item.$input.val())) {
						...
						return true
					} else */if (item.barcode == me.search_item.$input.val()) {
						search_status = false;
						return item.barcode == me.search_item.$input.val();
	...
	/*apply_category: function() {
		var me = this;
		category = this.quotation_type_field.$input.val();
	...
	bind_qty_event: function () {
		...
		$(this.wrapper).find(".pop-item-grade").on("change", function () {
		...
		$(this.wrapper).find("[data-action='increase-qty']").on("click", function () {
			var item_code = $(this).parents(".pop-bill-item").attr("data-item-code");
			var qty = flt($(this).parents(".pop-bill-item").find('.pop-item-qty').val()) + 1;
			me.update_qty(item_code, qty)
		})
	...
	update_rate: function () {
		...
		$(this.wrapper).find(".pop-item-rate").on("change", function () {
			var item_code = $(this).parents(".pop-bill-item").attr("data-item-code");
	...
	farmer_validate: function () {
		var me = this;
		if (!this.frm.doc.farmer) {
			frappe.throw(__("Please select farmer"))
		}
	},
	...
	add_to_cart: function () {
		var me = this;
		var caught = false;
		var no_of_items = me.wrapper.find(".pop-bill-item").length;

		this.farmer_validate();
	...
	add_new_item_to_grid: function () {
		...
		this.child.cost_center = this.items[0].cost_center;//this.pos_profile_data['cost_center'] || this.items[0].cost_center;
		this.child.income_account = this.items[0].income_account;//this.pos_profile_data['income_account'] || this.items[0].income_account;
		this.child.warehouse = (this.item_serial_no[this.child.item_code]
			? this.item_serial_no[this.child.item_code][1] : this.items[0].default_warehouse);//(this.pos_profile_data['warehouse'] || this.items[0].default_warehouse));
		//this.child.price_list_rate = flt(this.price_list_data[this.child.item_code], 9) / flt(this.frm.doc.conversion_rate, 9);
		this.child.rate = 0;//flt(this.price_list_data[this.child.item_code], 9) / flt(this.frm.doc.conversion_rate, 9);
		...
		this.child.item_tax_rate = 0;//JSON.stringify(this.tax_data[this.child.item_code]);
	},
	...
	show_items_in_item_cart: function () {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			$(frappe.render_template("pop_bill_item", {
		...
		this.wrapper.find("input.pop-item-qty").on("focus", function () {
			$(this).select();
		});

		this.wrapper.find("input.pop-item-discount").on("focus", function () {
			$(this).select();
		});

		this.wrapper.find("input.pop-item-rate").on("focus", function () {
			$(this).select();
		});
	},
	...
	set_taxes: function () {
		...
		$.each(taxes, function (i, d) {
			if (d.tax_amount && cint(d.included_in_print_rate) == 0) {
				$(frappe.render_template("pop_tax_row", {
	...
	disable_input_field: function () {
		...
		$(this.wrapper).find('.pop-bill').css('pointer-events', pointer_events);
		$(this.wrapper).find('.pop-items-section').css('pointer-events', pointer_events);
		this.set_primary_action();
	},
	...
	create_invoice: function () {
		...
		if (this.name) {
			...
			this.name = $.now();
			this.frm.doc.offline_pop_name = this.name;
			this.frm.doc.transaction_date = frappe.datetime.get_today();
			//this.frm.doc.posting_time = frappe.datetime.now_time();
			invoice_data[this.name] = this.frm.doc
			this.fq_docs.push(invoice_data)
			this.update_localstorage();
			this.set_primary_action();
		}
		return invoice_data;
	...
	update_invoice: function () {
		...
		$.each(this.fq_docs, function (index, data) {
			for (key in data) {
				if (key == me.name) {
					me.fq_docs[index][key] = me.frm.doc;
	...
	update_localstorage: function () {
		try {
			localStorage.setItem('farmer_quotation_doc', JSON.stringify(this.fq_docs));
		} catch (e) {
	...
	get_doc_from_localstorage: function () {
		try {
			return JSON.parse(localStorage.getItem('farmer_quotation_doc')) || [];
		} catch (e) {
			return []
		}
	},

	set_interval_for_fq_sync: function () {
		var me = this;
		setInterval(function () {
			me.sync_farmer_quotation()
		}, 60000)
	...
	sync_farmer_quotation: function () {
		var me = this;
		this.fq_docs = this.get_submitted_quotation();

		if (this.fq_docs.length) {
			frappe.call({
				method: "fpo.fpo.doctype.farmer_quotation.pop.make_farmer_quotation",
				args: {
					doc_list: me.fq_docs
				},
				callback: function (r) {
	...
	get_submitted_quotation: function () {
		var invoices = [];
		var index = 1;
			...
			$.each(this.fq_docs, function (index, data) {
				for (key in data) {
					if (!in_list(me.removed_items, key)) {
						me.new_fq_docs.push(data);
					}
				}
			})
			this.fq_docs = this.new_fq_docs;
			this.update_localstorage();
		}
	},
    ...
    validate_quotation_type: function () {
		if (!this.frm.doc.quotation_type) {
			frappe.throw(__("Quotation Type is not specified."))
		}
	},
	...
	get_actual_qty: function (item) {
		...
		var warehouse = item.default_warehouse;//this.pos_profile_data['warehouse'] || item.default_warehouse;

...
#	update_multi_mode_option(doc, pos_profile)
	...
	return {
		'doc': doc,
#		'default_customer': pos_profile.get('farmer'),
		'items': get_items_list(),
#		'item_groups': get_item_group(),
...
def update_pos_profile_data(doc, pos_profile, company_data):
	...
	# doc.customer_group = pos_profile.get('customer_group') or get_root('farmer Group')
...
#def get_quotation_types(pos_profile):
#	if pos_profile.get('item_groups'):
#		item_groups = []
#		for d in pos_profile.get('item_groups'):
#			item_groups.extend(get_child_nodes('Item Group', d.item_group))
...
def get_farmers_list():
	...
	# if pos_profile.get('customer_groups'):
		# Get customers based on the farmer groups defined in the POS profile
		# for d in pos_profile.get('customer_groups'):
			# customer_groups.extend(get_child_nodes('farmer Group', d.customer_group))
...
def validate_farmer(doc):
	if not frappe.db.exists('Farmer', doc.get('farmer')):
		...
		#farmer_doc.customer_type = 'Company'
		#farmer_doc.customer_group = frappe.db.get_single_value('Selling Settings', 'customer_group')
		#farmer_doc.territory = frappe.db.get_single_value('Selling Settings', 'territory')
		#farmer_doc.flags.ignore_mandatory = True
		...
		doc['farmer'] = farmer_doc.name
			...
			#make_address(doc, args, farmer_doc.name)

# def make_address(doc, args, farmer):
